...
st.write(
    "Class 12 Corporate Security Simulation Project (Powered by NLP & Risk"
    " Analysis)"
)
st.markdown("---")
...
if data_results is None:
  ...
  if st.button("🚀 Run AI Scan Risk Analysis"):
    if not text_input.strip():
      ...
      # 1. Unrealistic Financial Payout Check
      ...
      if has_unrealistic_amount or (
        ...
        triggered_features.append(
            "🚨 Unrealistic Payout Anomaly: Unreasonably high financial"
            " promise/daily payout detected."
        )
      ...
      if word_count < 10:
        risk_score += 35
        triggered_features.append(
            f"⚠️ Structural Anomaly: Description is too short ({word_count}"
            " words). Lacks job role details."
        )
      ...
      for bad_word in PROFANITY_KEYWORDS:
        if bad_word in text_lower or bad_word in email_lower:
          risk_score += 50
          triggered_features.append(
              "⛔ Content Violation: High-risk/abusive term detected"
              f" ('{bad_word}')."
          )
      ...
      if any(domain in email_lower for domain in SUSPICIOUS_DOMAINS):
        risk_score += 20
        triggered_features.append(
            "📧 Sender Domain Alert: Public email domain (Gmail/Yahoo/Outlook)"
            " used instead of corporate server."
        )
      ...
      # Prominent Risk Analysis Section
...
st.write(
    "Improve your LinkedIn profile and discover skills, projects, and"
    " certifications to increase your internship opportunities."
)
...
headline = st.text_input(
    "LinkedIn Headline", placeholder="Example: AI Student | Python Developer"
)
